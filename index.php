...
$select = $modx->db->select('*', $modx->getFullTableName('exchange1c_manager_config'));
if ($modx->db->getRecordCount($select)) {
    $select = $modx->db->makeArray($select);
    foreach ($select as $item) {
        $config[str_replace("conf_", "", $item['setting'])] = $item['value'];
    }
}
...
if (!isset($_SERVER['PHP_AUTH_USER']) or $_SERVER['PHP_AUTH_USER']!==$user or $_SERVER['PHP_AUTH_PW']!==$pass) {
    ...
    if (isset($_REQUEST['mode']) && $_REQUEST['type'] == 'catalog') {
        $config['dir_temp'] = MODX_BASE_PATH . MGR_DIR . '/1c_exchange/temp/';
